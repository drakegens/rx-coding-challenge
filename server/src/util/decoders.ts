...
export class DecodeError extends E.BaseError {
  plain_message: string;
  inner_error: DecodeError | null;

  constructor(message: string, inner_error: DecodeError | null = null) {
    super();
    ...
    (obj: any) => {
      ...
      try {
        ...
        throw new DecodeError(
          `Expected the value to ${expectation} but got ${actualValueDescription(
            obj
          )}. ${e.plain_message || ""}`,
          e
        );
...
export function required<T>(path: string, decoder: Decoder<T>): Decoder<T> {
  ...
  return Object.assign(
    (obj: any) => {
      if (!_.has(obj, path)) {
        throw new DecodeError(
          `Expected the value at [${path}] to ${decoder.expectation} but there was none.`
        );
      }
      ...
      try {
        ...
        throw new DecodeError(
          `Expected the value at [${path}] to ${decoder.expectation}. ${
            e.plain_message || ""
          }`,
          e
        );
...
export const str: Decoder<string> = (function () {
  ...
  return Object.assign(
    (obj: any) => {
      if (!_.isString(obj)) {
        throw new DecodeError(
          `Expected to ${expectation} but got ${actualValueDescription(obj)}.`
        );
      }
...
export function arrayToString(
  ...
  const expectation = `be an array where each element should be a string`;
  return Object.assign(
    (obj: any) => {
      ...
      const strs: string[] = _.map(obj[path], str);
  ...
  return Object.assign(
    (obj: any) => {
      ...
      try {
        ...
        throw new DecodeError(
          `Expected the value with the key [${prop_name}] to ${
            decoder.expectation
          }. ${e.plain_message || ""}`
        );
      }
...
export type ObjectDecoder<T> = { [P in keyof Required<T>]: Decoder<T[P]> };

export function object<T extends Object>(props: ObjectDecoder<T>): Decoder<T> {
  const expectation = `be an object with the properties [${Object.keys(
    props
  ).join(", ")}]`;
  return Object.assign(
    (obj: any) => {
      ...
      try {
        ...
        for (prop_name of Object.keys(props)) {
          const decoder = (<any>props)[prop_name];
          result[prop_name] = decoder(obj);
        }

        return result;
      } catch (e) {
        const inner_error = _.isNil(e.inner_error)
          ? e.plain_message
          : e.inner_error.plain_message;
        throw new DecodeError(
          `Expected the value to ${
            (<any>props)[prop_name].expectation
          }. ${inner_error}`
        );
      }
    },
...
// export function timeRange<T>(boundDecoder: Decoder<T>): Decoder<TimeRange<T>> {
//    return object({
//       start_on: required('start_on', nullable(boundDecoder)),
//       end_on: required('end_on', nullable(boundDecoder)),
//       inclusive_start: required('inclusive_start', bool),
//       inclusive_end: required('inclusive_end', bool),
//       is_empty: required('is_empty', bool),
//    })
// }
...
export function dependent<T1, T2>(
  decoder: Decoder<T1>,
  dependentDecoder: (val: T1) => Decoder<T2>
): Decoder<T2> {
  return Object.assign(
    (obj: any) => {
      return dependentDecoder(decoder(obj))(obj);
    },
    { expectation: dependentDecoder(<any>null).expectation }
  );
}
